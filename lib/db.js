...
// Canlı ortamda DEMO_VERI=0 ile demo öğrenciler oluşturulmaz;
// komisyon hesabı her koşulda oluşturulur.
...
ensureFaq("Başvuru", "Staj başvurusu ne zaman yapılır?",
  "Yaz dönemi staj başvuruları 1 Haziran – 15 Temmuz tarihleri arasında kabul edilir. Dönem içi staj başvuruları her ayın 10. gününe kadar (10. gün dâhil) yapılmalıdır. Ayrıca kabul formunun staj başlangıcından en az 20 gün önce teslim edilmesi gerekir.");
ensureFaq("Vlog", "Vlog nasıl olmalı?",
  "Toplam en az 10 dakikalık, işyerinde çekilmiş kısa videolardan oluşmalı: yaptığın işler, deneyimlerin ve çalışma ortamın. Stajın ilk, orta ve son günlerinden bölümler içermeli — son gün birkaç fotoğrafla olmaz. İşyerinin izin verdiği bölümlerde, izin verdiği şekilde çek. Online stajda ekran görüntünün canlı aktığı video parçaları da olur. Videoyu YouTube gibi bir platforma yükleyip bağlantısını hem normal link hem QR kod olarak defterin son sayfasına ekle.");
...
function ensureUser(no, tc, name, email, sinif = 3) {
  if (!db.prepare("SELECT id FROM users WHERE ogrenci_no=?").get(no)) {
    db.prepare("INSERT INTO users (ogrenci_no, tc, name, email, password_hash, role, sinif) VALUES (?,?,?,?,NULL,'student',?)")
      .run(no, tc, name, email, sinif);
  }
}
// Hiç giriş yapmamış, sürece sıfırdan başlayacak öğrenciler.
// (Biri denemede "harcanırsa" diğerleriyle ilk giriş deneyimi tekrar yaşanabilir.)
if (DEMO) {
  ensureUser("20251004444", "55555555555", "Elif Aydın", "elif@ogr.balikesir.edu.tr");
  ensureUser("20251005555", "66666666666", "Yağmur Kaya", "yagmur@ogr.balikesir.edu.tr");
  ensureUser("20251006666", "77777777777", "Arda Demirtaş", "arda@ogr.balikesir.edu.tr");
  ensureUser("20251007777", "88888888888", "Defne Sarı", "defne@ogr.balikesir.edu.tr");
  ensureUser("20251008888", "11111111110", "Emir Yalçın", "emir@ogr.balikesir.edu.tr");
  ensureUser("20251009999", "22222222220", "Sude Aksoy", "sude@ogr.balikesir.edu.tr");
  ensureUser("20251010000", "33333333330", "Çınar Öztürk", "cinar@ogr.balikesir.edu.tr");
  ensureUser("20251011111", "44444444440", "Nehir Doğan", "nehir@ogr.balikesir.edu.tr");
  ensureUser("20251012222", "55555555550", "Atlas Şimşek", "atlas@ogr.balikesir.edu.tr");
  ensureUser("20251013333", "66666666660", "Mira Erdoğan", "mira@ogr.balikesir.edu.tr", 2); // 2. sınıf: tek başvuru kuralının demosu
}

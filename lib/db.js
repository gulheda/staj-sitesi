...
if (DEMO) {
  const taze = [
    ["20241001025", "20000000101", "Ela Yıldırım"],
    ["20241001026", "20000000202", "Rüzgar Koçak"],
    ["20241001027", "20000000303", "Lina Avcı"],
    ["20241001028", "20000000404", "Aras Güneş"],
    ["20241001029", "20000000505", "Masal Tekin"],
    ["20241001030", "20000000606", "Umut Bozkurt"],
  ];
  for (const [no, tc, ad] of taze) {
    if (!db.prepare("SELECT id FROM users WHERE ogrenci_no=?").get(no)) {
      db.prepare("INSERT INTO users (ogrenci_no, tc, name, email, password_hash, role, sinif) VALUES (?,?,?,?,NULL,'student',3)")
        .run(no, tc, ad, ad.toLowerCase().replace(/[^a-z]/g, "").slice(0, 10) + "@ogr.balikesir.edu.tr");
    }
  }
}

...
function isCounted(d, opts = {}) {
  ...
  const ctsSecili = opts.saturday || (opts.allowedDays && opts.allowedDays.includes(6));
  if (wd === 6 && !ctsSecili) return false;
...
function checkDates(startISO, endISO, opts = {}) {
  const { minDays = 20, allowedDays = null, saturday = false, minStart = null, tur = null } = opts;
  ...
  // Yaz stajı yaz aylarında yapılır: Ekim ve sonrasına taşamaz.
  if (tur === "yaz" && startISO) {
    const y = startISO.slice(0, 4);
    const sinir = `${y}-09-30`;
    if (startISO > sinir || (endISO && endISO > sinir)) {
      out.problems.push(`Yaz stajı en geç 30 Eylül'de bitmeli. Ekim ve sonrasında staj yapacaksan başvuruda "Dönem içinde" seçmelisin.`);
      return out;
    }
  }
